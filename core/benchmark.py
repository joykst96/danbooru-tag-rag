...
DEFAULT_QUERIES = [
    # --- 단어 1~2개 (짧은 쿼리: score 분포 + 별칭 직격 진단) ---
    "여캐",                          # 정확 별칭 단어. bigram 필요여부(#6) 핵심
    "트윈테일",                      # 별칭 직격
    "은발 적안",                     # 색 분화(#한계) + 복합 2속성

    # --- 색 분화 케이스 (HANDOFF가 구조적 한계로 인정한 영역) ---
    "물빛 머리",                     # aqua/cyan/light blue 중 뭘 끌어오나
    "청록색 눈동자",                 # teal/turquoise 분화

    # --- 적당히 짧게 (실사용 중간 길이) ---
    "교복 입은 트윈테일 소녀",
    "갑옷 입고 검 든 기사",

    # --- 장문 (Pass1 통문장 쿼리 버티는지 #2) ---
    "비 내리는 밤거리에서 우산 없이 서 있는 단발머리 소녀, 젖은 머리",
    "석양이 비치는 교실 창가에 앉아 책 읽는 안경 쓴 여학생",
]
